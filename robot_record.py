...
class TerrainAdaptiveController:
    ...
    ### P control: reacts to how big the error is right now, the bigger kp, the bigger the error the strongest the correction but may overshoot and oscillate too much --> sol: D control
    ...
    ### D control: reacts to the rate of the error over time, to counteract sharp or fast changes in th error So if the P-controller wants to suddenly increase PWM, the D-term smooths that reaction based on how quickly the error is already shrinking.
    def compute_pwm_output(self, current_speed, target_speed):
        error = target_speed - current_speed  # Compute the speed error
        current_time = time.time()  # Get current time in seconds

        #  Initialize previous error and time if not already done
        if not hasattr(self, 'prev_error'):
            self.prev_error = error
            self.prev_time = 1e-3

        #  Calculate time difference (dt) and error difference (de)
        dt = current_time - self.prev_time 
        if dt <=0 :
            dt = 1e-3
        de = error - self.prev_error

        #  Define your PD control constants
        Kp = 8.0  # Proportional gain
        Kd = 2.0   # Derivative gain (can be tuned based on system response)

        #  Compute the PWM output using both P and D terms
        ...
        #  Limit the PWM to a safe range (e.g. 0 to 100)
        return max(0, min(100, pwm))
